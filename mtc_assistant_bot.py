...
try:
    if GEMINI_API_KEY:
        ...
        # --- การปรับปรุง: เพิ่ม System Instruction และ Generation Config ---
        # 1. กำหนดบทบาทและแนวทางการตอบให้ AI
        system_instruction = "คุณคือ MTC Assistant ผู้ช่วย AI อัจฉริยะสำหรับนักเรียนมัธยมปลาย ให้ตอบคำถามอย่างเป็นมิตร สุภาพ และกระชับได้ใจความ"

        # 2. กำหนดค่าคอนฟิกสำหรับการสร้างข้อความ เพื่อป้องกันการใช้หน่วยความจำมากเกินไป
        generation_config = {
            "temperature": 0.7,
            "top_p": 1,
            "top_k": 1,
            "max_output_tokens": 1024, # จำกัดความยาวของคำตอบไม่ให้ยาวเกินไป
        }

        gemini_model = genai.GenerativeModel(
            'gemini-2.5-flash', # คงเวอร์ชันโมเดลเดิมตามโค้ดที่ให้มา
            generation_config=generation_config,
            system_instruction=system_instruction
        )

        app.logger.info("Gemini AI configured successfully with optimization.")
    else:
...
def get_next_class_info() -> str:
    ...
    return "วันนี้ไม่มีเรียนแล้วครับ กลับบ้านไปนอนได้ 🏠"
...
def get_gemini_response(user_message: str) -> str:
    """Gets a response from the Gemini AI model using streaming to save memory."""
    if not gemini_model:
        return "ขออภัยครับ ระบบ AI ของส่วนนี้ยังไม่สมบูรณ์"
    try:
        # --- การปรับปรุง: เปลี่ยนไปใช้ stream=True เพื่อทยอยรับข้อมูล ---
        response_stream = gemini_model.generate_content(user_message, stream=True)

        # รวมคำตอบที่ได้จาก stream แต่ละส่วนเข้าด้วยกัน
        full_response = ""
        for chunk in response_stream:
            # ตรวจสอบให้แน่ใจว่า chunk มี text ก่อนจะนำไปต่อกัน
        ...
        # หาก AI ไม่สามารถสร้างคำตอบได้ ให้ส่งข้อความ default กลับไป
