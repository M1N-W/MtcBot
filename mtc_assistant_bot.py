...
def get_music_link_message(user_message: str):
    ...
    # prompt ผู้ช่วยหาเพลง
    search_prompt = (
        f"คุณคือผู้ช่วยค้นหาเพลง กรุณาค้นหาลิงก์ YouTube สำหรับเพลงนี้: '{user_message}' "
        f"และตอบกลับมาเฉพาะลิงก์ YouTube ที่ถูกต้องลิงก์เดียวเท่านั้น ถ้าหาไม่เจอ ให้ตอบว่า 'ผมหาเพลงนี้ไม่เจอครับ'"
    )
    
    # เรียกใช้ฟังก์ชัน AI ที่มีอยู่แล้ว
    ai_response = get_gemini_response(search_prompt)
    
    # ตรวจสอบว่าคำตอบเป็นลิงก์หรือไม่
